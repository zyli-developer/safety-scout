...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector('text=拍隐患', { timeout: 5_000 }).catch(() => {
      failures.push('找不到 "拍隐患" 按钮 —— BigButton 没渲染');
    });
    ...
    const buttonText = await page.locator('text=拍隐患').first().textContent().catch(() => null);
