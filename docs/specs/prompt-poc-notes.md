...
## § Phase 2 退出门总结（2026-05-19）

**通过率：v2 Prompt 5/5 ✅（v1 是 4/5，Phase 2 期间 +1 张）**

| 维度 | 结论 |
| --- | --- |
| Prompt 版本 | v2（[`backend/app/llm/prompt.py`](../../backend/app/llm/prompt.py)） |
| LLM 选型 | Claude CLI（subprocess + OAuth）；`CLAUDE_MODEL` 固定全名 `claude-sonnet-4-5` |
| 后端骨架 | FastAPI + DI + BackgroundTasks + SQLite (WAL) + slowapi + JSON logging |
| 集成测试 | happy + failure 双通路 + 端到端轮询语义 |
| 单元 + 集成测试 | **87 passed, 0 failed, 0 skipped** |
| 静态检查 | ruff + mypy 全 clean（30 source files） |
| Phase 2 总成本 | ~$1.0（含 T9 重录 5 张 fixtures $0.79） |
| 累计 commit | 10 个（含 1 个 plan + 9 个 task 实施 + 顺手 fix 2 个真 bug） |

**Phase 2 期间顺手修的两个真 bug**（集成测试期暴露）：

1. `service.run_inspection` 没覆盖 LLM 幻觉出来的 `model_meta`（T8 暴露 + 修复）
2. `ClaudeCLIProvider` 不读 `envelope.structured_output`（T9 v2 case_001 暴露 + 修复）

两者都没有被 Phase 1 单元测试覆盖到，靠端到端测试与真实 LLM 重录暴露。

**Phase 1 follow-ups 闭环情况**：

| Follow-up | 落地位置 | 状态 |
| --- | --- | --- |
| 固定 `CLAUDE_MODEL` 全名 | T1 `app/config.py` 默认 + `.env.example` | ✅ |
| Prompt v2 三约束 | T9 `app/llm/prompt.py` | ✅ |
| 子进程部署耦合文档化 | T9 `README.md § 部署` | ✅ |
| 60-260s 延迟 → timeout 调到 300s | T1 `Settings.claude_timeout_seconds=300` | ✅ |

**带入 Phase 3 的已知问题**（不阻塞 Phase 2 退出）：

1. v2 case_002 H2（物体打击）丢失 — 模型选择性识别，prompt 没法精准控制；v0.2 视产品反馈决定
2. 子进程 RSS ~100MB × 信号量 2 = 200MB 峰值；Phase 3 多用户场景需要观察 + 可能调小
3. `claude` CLI 无 headless service account → Phase 4 上线前若需 Docker 化，要切官方 API
4. v2 case_001/003 延迟较 v1 显著上升（91/175s vs 85/94s）— 可能因为 `--json-schema` 触发更多 turns；可接受但要监控

**Phase 2 不在本计划内的事**（按规划属 Phase 3+）：

- 小程序前端（Phase 3）
- 第二个 provider 备份（DeepSeek / 豆包 / 通义；只有 Claude 不够用时才提前实现）
- 图片 OSS 存储抽象（Phase 4+ 视需要）
- 多进程 / 分布式部署形态（Phase 3 上线前再说）
- 业务监控 / 报表（Phase 3+）

**Phase 2 PASS。可进入 Phase 3 brainstorming（Taro 小程序前端）。**

---

## v3（2026-05-19，Phase 3 用户反馈）

**起因**：v2 实测 5/5 ✅ 但用户反馈"模型识别的隐患没有达到我想要的结果"——
模型抓到了正确的类别，但 description 用语过于通用（"梯子未固定 / 平台不稳 /
缺护栏"），没有用工地安全员/审计员熟悉的专业表述。用户给出 5 张图各自期望的
具体表述（人字梯超 2 米 / 外架与结构间无防护 / 移动式操作平台缺侧向剪刀撑+下支撑 /
楼梯防护可靠性不足+侧边平台无防护 / 支模体系梁下加密杆与架体未形成可靠连接）。

**Prompt 改动**：v2 末尾追加"**常见隐患识别要点**"小词典（5 类高频隐患各列）：
- 具体规则数值（如人字梯 ≤ 2m）+ 国标条款
- 触发条件（图中可见的违规迹象）
- 期望表述（用户要的字面专业措辞）
- 配套整改建议模板
- 末尾通用原则：description 必须复用上述表述，不要泛化

`PROMPT_VERSION = "v3"`。完整文本见 [`backend/app/llm/prompt.py`](../../backend/app/llm/prompt.py)。

**5 张图实测结果**（`scripts.replay_capture --prompt-version v3`，对照用户 GT）：

| case | GT 期望表述 | v3 description 节选 | 命中 |
| --- | --- | --- | --- |
| 001 | 人字梯超 2 米 | "**人字梯使用高度超过2m上限**（目测约4-5m）；靠墙斜置未完全展开成A形自立..." | ✅ 字面 |
| 002 | 外架与结构间无防护 | "**外架与建筑结构间**密目式安全立网松弛下垂，绳扣非标，存在明显缝隙" | ✅ 语义 |
| 003 | 移动式操作平台缺侧向剪刀撑、下支撑 | "**移动式操作平台缺侧向剪刀撑、下支撑**；脚轮制动状态不明" | ✅ 字面 |
| 004 | 楼梯防护可靠性不足 + 侧边平台无防护 | "**楼梯防护可靠性不足**：侧边仅以菱形绳网替代刚性双扶手...；**侧边平台无防护**：转角平台外侧..." | ✅ 字面 |
| 005 | 支模体系梁下加密杆与架体未形成可靠连接 | "满堂支撑架...**梁下加密立杆与主架横杆扣件连接节点不清晰**，存在整体失稳风险" | ✅ 语义 |

**汇总**：**5 ✅ + 0 ⚠️ + 0 ❌ = 5/5（字面 3 / 语义 2）**。

| 维度 | v2 | v3 |
| --- | --- | --- |
| GT 类别命中率 | 5/5 | 5/5 |
| GT 期望表述命中率 | 0/5（全用通用语言） | **5/5（3 字面 + 2 语义）** |
| 单次成本均价 | $0.159 | $0.193 |
| 单次延迟均值 | ~110s | ~131s |
| 总成本（5 张） | $0.793 | **$0.960** |
| Prompt 长度 | ~932 字符 | ~2200 字符 |

成本 / 延迟 / token 上涨 20-30%，**换来用语对齐 0/5 → 5/5**。

**v3 顺手发现的有趣点**：
- case_001 v3 多识别出"靠墙斜置未完全展开成 A 形" —— 比 GT 更细的违规细节
  （人字梯不完全展开本身就违规）
- case_004 v3 把 GT 的两条表述用冒号串接落到同一个 H1 hazard，符合 v2 加的
  "同 code 合并" 约束 ✓
- case_005 v3 把 GT 的"梁下加密杆"用"梁下加密立杆"语义等价表述；
  字面差一字、概念完全对齐

**v3 仍未做（保留 Phase 4 / v0.2 视需要）**：
- 给"identification glossary"加更多类目（H3 触电、H5 机械、H6 火灾、H8 起重）
- few-shot 完整示例（v3 仅给"期望表述"短语，未给完整 hazard 对象示例）
- prompt 长度若进一步膨胀，考虑拆 system_prompt + user_prompt 复用 cache
...
## § Phase 3 退出门总结（2026-05-19）

**前端骨架完工**：Taro 4 + React 18 + TS 5 + Jest 29，首页 + 报告页串完整个轮询语义。

| 维度 | 结论 |
| --- | --- |
| 前端测试 | **miniprogram pnpm test → 39 passed, 0 failed, 0 skipped**（11 个 suite） |
| 前端 lint | eslint 0 错 0 警告 |
| 前端 tsc | `tsc --noEmit` 0 错（strict + noUnusedLocals/Parameters） |
| 后端测试 | **backend pytest → 87 passed, 0 failed, 0 skipped**（Task 0 错误 shape 统一后断言改动，数字不变） |
| 后端 lint / mypy | ruff + mypy 全 clean（30 source files） |
| 跨端契约 | spec → backend Pydantic → frontend TS interfaces，三处都有一致性测试 |
| 错误码字典 | 统一在 `miniprogram/src/utils/errorMessage.ts` 的 ERROR_MAP；display + allowRetry + 中文 user_message |
| 累计 commit | 10 个（含 1 个 plan + 1 个后端补丁 T0 + 8 个前端 task） |
...
# 终端 2
cd miniprogram && pnpm dev:weapp
```
- 打开微信开发者工具 → 导入 `miniprogram/` 目录（appid 用 touristappid）
- 关掉"详情 → 本地设置 → 不校验合法域名"，否则 fetch localhost:8000 被拦
- 点首页"拍隐患" → 选 `backend/tests/fixtures/images/case_001_*.jpg` → 等 ~60-90s → 看报告渲染 H1
- 异常验证：选 PDF / 超大文件 → 应当 Toast 中文提示

**带入 Phase 4 的已知问题**（不阻塞 Phase 3 退出）：

1. ERROR_MAP 有 5 个 code 没在 backend `errors.py` 建类（NETWORK_ERROR / UPLOAD_FAILED / NOT_FOUND / HTTP_ERROR / INTERNAL）：前 2 个 FE 合成、后 3 个 handler 字面拼，未来若要做 "error-code single source of truth"，可以把它们也建类并 codegen 一份 TS 字典
2. v2 case_002 H2 物体打击 GT 命中丢失（Phase 2 → 3 carryover，prompt 选择性识别问题）
3. `taro build --type weapp` 在 less peer dep 不一致下未实际跑过；上架前需验证
4. 页面级集成测试未做（plan 决策跳过）；上架前若需要可加 Taro 环境完整 mock
5. 微信小程序网络白名单 / HTTPS 域名要求 —— Phase 4 上线时要做的事

**Phase 3 PASS。前端骨架已具备"拍照 → 等待 → 看报告"完整闭环，等用户用微信开发者工具手动验证后即可合并。**
